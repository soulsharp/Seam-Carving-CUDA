...
from utils.pycuda_utils import get_cuda_kernels_src_file, load_image, get_backward_seam_from_idx
     
if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    default_image_path = os.path.join(os.getcwd(), "examples", "images", "joel-filipe-QwoNAhbmLLo-unsplash.jpg")

    # Arguments provided by user
    parser.add_argument("--img_path", type=str, help="Path of image to be resized", default=default_image_path)
    parser.add_argument("--resized_width", type=int, help="Width of the resized image", default=400)
    parser.add_argument("--resized_height", type=int, help="Height of the resized image", default=400)
    ...
    numBlocks = ((image_width + 2 + threadsPerBlock[0] - 1) // threadsPerBlock[0], (image_height + 2 + threadsPerBlock[1] - 1) // threadsPerBlock[1], 1)
    ...
    dummy_output = np.zeros(image_height, dtype=np.int32)
    ...
    print("Rgb to gray kernel executed successfully! Time:", end_time - start_time)
    ...
    start_time = time.time()
    cumulative_energy_fn_backward(d_energy_map, d_cumulative_energy_map, d_dummy_output,
                np.int32(image_height), np.int32(image_width),
                block=threadsPerBlockCumulative,
                grid=numBlocksCumulative, shared=shared_mem_size)
    cuda.Context.synchronize()
    end_time = time.time()
    print("Kernel executed successfully! Time:", end_time - start_time)
    ...
    # Finds the minimum element in each threadblock, np.argmin further reduces it
    ...
    start_time = time.time()
    seam = get_backward_seam_from_idx(backtrack_idx, cumulative_map.reshape(image_height, image_width))
    end_time = time.time()
